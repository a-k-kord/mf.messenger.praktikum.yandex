...
export function validateLimitedString(event: Event, minLength: number = 3, maxLength: number = 100) {
...
export function validatePasswordConfirm(event: Event, minLength: number = 3, maxLength: number = 100) {
    const input: HTMLInputElement = (<HTMLInputElement>event.target);
    if(!input.value.length) {
        this.childBlocks.error.setProps({text: 'Необходимо заполнить', isHidden: false})
    } else {
